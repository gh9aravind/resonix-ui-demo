...
private fun ResonixUiPlayground() {
    ...
    // This ticker just advances a fake position while "isPlaying" is true,
    // purely so the NowPlayingScreen's progress UI has something to animate.
    val track = DemoTrackCatalog.current
    ...
    LaunchedEffect(isPlaying) {
        while (isPlaying) {
            delay(500)
            progressMs = if (progressMs + 500 >= track.durationMs) {
                0L
            } else {
                progressMs + 500
            }
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        when (currentScreen) {
            ...
            PreviewScreen.Permissions -> PermissionScreen(
                onContinue = { currentScreen = PreviewScreen.NowPlaying }
            )
            PreviewScreen.NowPlaying -> NowPlayingScreen(
                ...
                onSeek = { progressMs = it.coerceIn(0L, track.durationMs) },
                onBack = { currentScreen = PreviewScreen.Permissions }
            )
        }
    }
}
